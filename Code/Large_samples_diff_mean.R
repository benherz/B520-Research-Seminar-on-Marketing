...
for(i in 1:length(control_sizes)){
  simulation_params <- list(N = control_sizes[i], T = T, n_treated = n_treated, treatment_period = treatment_period,
                            ...
                            treatment_effect = treatment_effect, treated_mean = 5)
  ...
  # Save example df
  write.csv(df, paste0(path, "/Output/Data/example_df_N_", N, "_diff_mean.csv"))
  ...
  plot_grouped_simulations(results$dfs) %>%
    ggsave(filename = paste0("Output/Plots/data_plot_N_", control_sizes[i],
                             "_n_treated_", treated_sizes[i], "_diff_mean.png"), width = 10, height = 5, units = "in", dpi = 300)
