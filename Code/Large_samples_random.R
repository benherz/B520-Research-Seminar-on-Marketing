...
for(i in 1:length(control_sizes)){
  ...
  # Save example df
  write.csv(df, paste0(path, "/Output/Data/example_df_N_", N, "_random.csv"))
  ...
  plot_grouped_simulations(results$dfs) %>%
    ggsave(filename = paste0("Output/Plots/data_plot_N_", control_sizes[i],
                             "_n_treated_", treated_sizes[i], "random.png"), width = 10, height = 5, units = "in", dpi = 300)

}

write.csv(time_df, "Output/Data/time_df_large_sample_random.csv")
write.csv(all_estimates, "Output/Data/all_estimates_large_sample_random.csv")
