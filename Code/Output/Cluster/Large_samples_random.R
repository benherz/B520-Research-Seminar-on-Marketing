...
for(i in 1:length(control_sizes)){
  ...
  # Save example df
  write.csv(df, paste0("Output/Data/example_df_N_", N, "_random.csv"))
  ...
  plot_grouped_simulations(results$dfs) %>%
    ggsave(filename = paste0("Output/Plots/data_plot_N_", N,"_random.png"), width = 10, height = 5, units = "in", dpi = 300)
